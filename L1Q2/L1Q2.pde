...
float HY = 0;
float ROC = 4;//Raio do ombro ao cotovelo
float RCH = 6; //Raio do CUtovelo a mão
//velocidade angular
float tetag = PI/720;
float alphag = PI/360;
//270 graus
...
void draw(){
  ...
  if(teta < (5*PI)/3){ //rodar cutuvelo 
    teta += tetag;
    teta2 += tetag; //Incrementa a mao com a velocidade do ombro
